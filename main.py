...
class Dawn:
    ...
    def log(self, message):
        print(
            f"{Fore.CYAN + Style.BRIGHT}[ {datetime.now().astimezone(wib).strftime('%x %X %Z')} ]{Style.RESET_ALL} AHMEDTAREK"
...
╚═╝  ╚═╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝╚═════╝        ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
    ...
    async def question(self):
        while True:
            try:
                print(f"{Fore.BLUE+Style.BRIGHT}SELECT ANY ONE TO CONTINUE - No Selling")
                print("1. Run With Auto Proxy - 🔴 BAN")
                print("2. Run With Manual Proxy- 🟡 Moderate")
                print("3. Run Without Proxy - 🟢 SAFE")
                print(" CRYPTO 3RB COMMUNITY FIRST  ")
